...
- **There is no undo — snapshot instead.** Ghidra *discards its undo history on every save* (`BufferMgr.doSetSourceFile` packs all checkpoints into the baseline; `ProgramDBChangeSet.clearUndo`), and this server auto-saves after every mutating call. So by the time a tool call returns, its damage is already on disk and nothing can take it back — the GUI's Ctrl+Z included. Before a risky bulk write, take a backup with `manage_files op=copy` (`migrate` does this automatically and reports the path). To restore: delete the damaged file, copy the backup back, rename it.
